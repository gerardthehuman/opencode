...
export function queuePendingProxyCall(
  ...
  if (previous) {
    ...
    previous.reject(new Error(`Replaced pending proxy call ${call.id} with a fresh one`));
    pendingByCallId.delete(call.id);
    indexRemove(previous.sessionKey, call.id);
  }

  const deadlineMs = resolveProxyCallTimeoutMs(call.toolName, call.input, timeoutOverrides);
...
export function resolvePendingProxyCallById(toolCallId: string, result: ProxyToolResult): boolean {
...
export function rejectPendingProxyCallById(toolCallId: string, error: Error): boolean {
  const pending = pendingByCallId.get(toolCallId);
  if (!pending) return false;
  pendingByCallId.delete(toolCallId);
  indexRemove(pending.sessionKey, toolCallId);
...
export function rejectAllPendingProxyCallsForSession(sessionKey: string, error: Error): number {
  const s = callIdsBySession.get(sessionKey);
  if (!s) return 0;
  const ids = [...s];
  let count = 0;
