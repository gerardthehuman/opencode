 ...
 * call 401s.
 */
function substituteEnvPlaceholders(source: Record<string, unknown>): Record<string, string> {
...
function translateServer(
  ...
  if (type === "local") {
    ...
    if (spec.environment && typeof spec.environment === "object") {
      out.env = substituteEnvPlaceholders(spec.environment as Record<string, unknown>);
    }
    return out;
  }
  ...
  if (type === "remote") {
    ...
    if (spec.headers && typeof spec.headers === "object") {
      out.headers = substituteEnvPlaceholders(spec.headers as Record<string, unknown>);
    }
    return out;
  }
...
function extractMcpBlock(config: Record<string, unknown>): Record<string, OpencodeServer> {
 ...
 */
export function mergeOpencodeMcp(cwd: string, runtimeStatus?: RuntimeMcpStatus): MergedMcp {
...
function finishBridge(input: {
  ...
  const outPath = path.join(pluginTmpDir(), `mcp-${hash}.json`);
  try {
