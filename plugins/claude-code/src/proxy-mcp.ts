...
export function isExpectedCleanupError(message: string): boolean {
  return (
    (message.includes("timed out after") && message.includes("waiting for opencode to resolve")) ||
...
function lookupCaseInsensitive(map: Record<string, number>, key: string): number | undefined {
 ...
 */
export function resolveProxyClientCeilingMs(overrides: Record<string, number> | undefined): number {
 ...
 */
export function extractAgentTypeList(liveDescription: string | undefined): string | undefined {
  ...
  for (const raw of live.slice(start).split("\n")) {
    ...
    entries.push(
      `- ${name}: ${
        blurb.length > AGENT_BLURB_LIMIT ? `${blurb.slice(0, AGENT_BLURB_LIMIT).trimEnd()}…` : blurb
      }`,
    );
  }
...
export function overlayTaskProxyDescription(
  ...
  return tools.map((t) =>
    t.name === "task" ? { ...t, description: `${agentTypes}\n\n${t.description}` } : t,
  );
}
...
export function overlayQuestionProxyDescription(
  ...
  return tools.map((t) =>
    t.name === "question" ? { ...t, description: `${live}\n\n${QUESTION_PROXY_NOTE}` } : t,
  );
}
...
export async function createProxyMcpServer(
    ...
    try {
      ...
      if (request.method === "tools/call") {
        ...
        const result = await new Promise<ProxyToolResult>((resolve, reject) => {
          ...
          const deadlineMs = resolveProxyCallTimeoutMs(toolName, input, timeoutOverrides);
          timer = setTimeout(() => {
  ...
  const api: ProxyMcpServer = {
    ...
    configPath() {
      ...
      const outPath = path.join(pluginTmpDir(), `proxy-${hash}.json`);
