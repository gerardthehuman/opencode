...
function renderMessageContentForCompaction(msg: any): { text: string; toolResultCount: number } {
  const lines: string[] = [];
  ...
  for (const part of msg.content as any[]) {
    ...
    switch (part.type) {
      case "text":
        ...
        break;
      case "tool-call":
        lines.push(`[tool_use:${part.toolName ?? "unknown"}(${renderToolInput(part.input)})]`);
        break;
      case "tool-result":
        toolResultCount++;
        lines.push(
        ...
        break;
      case "image":
        lines.push(`[image: ${part.mediaType ?? part.mimeType ?? "unknown"}]`);
        break;
      case "file":
        lines.push(`[file: ${part.mediaType ?? part.mimeType ?? "unknown"}]`);
        break;
      case "reasoning":
...
function buildCompactionHistory(prompt: Prompt): string | null {
  ...
  const end =
    prompt.length > 0 && prompt[prompt.length - 1].role === "user"
      ? prompt.length - 1
      : prompt.length;
