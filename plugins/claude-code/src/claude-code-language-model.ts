...
import { getRuntimeMcpStatus, fetchOpencodeToolList, resolveSpawnCwd } from "./runtime-status.js";
import {
  getActiveProcess,
  setActiveProcess,
  spawnClaudeProcess,
...
export function resolveSessionAffinity(
  ...
  if (providerOptions) {
    const bag = (providerOptions as any)[providerKey] ?? (providerOptions as any)["claude-code"];
 ...
 */
export function hasNewUserContent(prompt: LanguageModelV3CallOptions["prompt"]): boolean {
...
function formatAskUserQuestion(input: Record<string, unknown>): string {
  ...
  const questions: any[] = Array.isArray(anyInput?.questions) ? anyInput.questions : [];
...
function looksLikeBlocker(text: string): boolean {
  ...
  return /\b(blocked|blocker|cannot proceed|can't proceed|unable to proceed|need clarification|need more information|permission denied|failed and needs|requires your|needs your|needs you to|action required|manual step|required from you)\b/.test(
    normalized,
  );
}
...
function looksLikeFinalAnswer(text: string): boolean {
  ...
  return (
    /\b(done|completed|fixed|implemented|verified|published|released|sent|delivered|updated|shipped|deployed|merged|tagged|live|pinned)\b/.test(
      normalized,
    ) ||
 ...
 */
function extractSystemMessages(prompt: LanguageModelV3CallOptions["prompt"]): string[] {
  const out: string[] = [];
...
export class ClaudeCodeLanguageModel implements LanguageModelV3 {
  ...
  private toFinishReason(reason: "stop" | "tool-calls" = "stop"): LanguageModelV3FinishReason {
    return {
      unified: reason,
      raw: reason,
    };
  ...
  private requestScope(options: { tools?: unknown }): "tools" | "no-tools" {
    ...
    if (tools && typeof tools === "object") {
      return Object.keys(tools as Record<string, unknown>).length > 0 ? "tools" : "no-tools";
    }
    return "no-tools";
  }
  ...
  private resolvedProxyTools(): ProxyToolDef[] | null {
    ...
    const defsByName = new Map(DEFAULT_PROXY_TOOLS.map((t) => [t.name.toLowerCase(), t]));
  ...
  private async resolvedProxyMcpTools(
    ...
    const items = await fetchOpencodeToolList(this.config.provider, this.modelId, this.config.cwd);
    ...
    const serversByLengthDesc = [...allEnabledServerNames].sort((a, b) => b.length - a.length);
   ...
   *   3. `"default"`.
   */
  private sessionAffinity(options: LanguageModelV3CallOptions): string {
    const headers = (options as any)?.headers as Record<string, string | undefined> | undefined;
  ...
  private handleControlRequest(
    ...
    if (request.subtype === "can_use_tool") {
      ...
      if (behavior === "allow") {
        ...
        const denyMessage = denyMessageForTool(toolName, this.config.controlRequestDenyMessage);
        this.writeControlResponse(proc, requestId, {
          behavior: "deny",
          message: denyMessage,
          toolUseID: request.tool_use_id,
  ...
  private getReasoningEffort(
    ...
    const bag = (providerOptions as any)[ownKey] ?? (providerOptions as any)["claude-code"];
  ...
  private isCompactionCall(options: LanguageModelV3CallOptions): boolean {
  ...
  private thinkingCliOptions(): {
    ...
    return {
      thinking: "enabled",
      thinkingDisplay:
        process.env.CLAUDE_CODE_SHOW_THINKING_SUMMARIES === undefined ? "summarized" : undefined,
    };
  }

  private latestUserText(prompt: LanguageModelV3CallOptions["prompt"]): string {
    for (let i = prompt.length - 1; i >= 0; i--) {
      const msg = prompt[i];
      if (msg.role !== "user") continue;
  ...
  private synthesizeTitle(prompt: LanguageModelV3CallOptions["prompt"]): string {
  ...
  async doGenerate(
    ...
    if (
      ...
      (this.resolvedProxyTools() ||
        (this.config.proxyOpencodeMcpTools !== false && this.config.bridgeOpencodeMcp !== false))
    ) {
    ...
    const hasPriorConversation =
      options.prompt.filter((m) => m.role === "user" || m.role === "assistant").length > 1;
    ...
    const userMsg = getClaudeUserMessage(options.prompt, includeHistoryContext, reasoningEffort);
    ...
    const cliArgs = buildCliArgs({
      ...
      disallowedTools: this.config.webSearch === "disabled" ? ["WebSearch"] : undefined,
    ...
    const toolCallStreams = new Map<number, { id: string; name: string; inputJson: string }>();
  ...
  async doStream(
    ...
    const hasMatchedPendingResults = previousPendingProxyMatches.some((m) => m.result !== null);
      ...
      start(controller) {
        ...
        const setup = async () => {
          ...
          if (compactionMode) {
            ...
            const discovery = self.effectiveMcpConfig(cwd, undefined, runtimeStatus!);
            ...
            // bridging.
            const proxyMcpTools = await self.resolvedProxyMcpTools(discovery.allEnabledServerNames);
            ...
            if (enrichedProxy && taskProxyEnabled) {
              ...
              log.info("task proxy description overlay", {
                ...
                listsAgentTypes: Boolean(
                  liveToolInfo.taskDescription?.includes("Available agent types"),
                ),
              });
            }
            ...
            const combinedList = [...(enrichedProxy ?? []), ...(proxyMcpTools ?? [])];
            ...
            const systemPromptFile = activeProcess
              ? undefined
              : buildAppendedSystemPrompt(cwd, self.config.multiStepContinuation !== false, [
                  ...extractSystemMessages(options.prompt),
          ...
          const onStartWatchdogFire = () => {
            ...
            if (respawnAttempted) {
              log.error("claude process still silent after respawn; ending turn", {
                sessionKey: sk,
              });
              deleteActiveProcess(sk);
              deleteClaudeSessionId(sk);
            ...
            if (!newAp) {
              log.error("no active process to respawn (start watchdog); ending turn", {
                sessionKey: sk,
              });
              controllerClosed = true;
              cleanupTurn();
          ...
          const scheduleResultBoundary = (completeResult: () => void, delayMs: number) => {
          ...
          const completeResult = (msg: ClaudeStreamMessage) => {
            ...
            const autoDecision = shouldAutoContinueIncompleteTurn(autoContinueState, {
          ...
          const lineHandler = (line: string) => {
            ...
            try {
              ...
              if (msg.type === "assistant" && msg.message?.content && !gotPartialEvents) {
                const hasText = msg.message.content.some((b: any) => b.type === "text" && b.text);
              ...
              if (msg.type === "result") {
                ...
                if (drainBuffer.length > 0 && shouldDeferResult) {
                  log.info("waiting for parallel proxy calls at turn-result boundary", {
                    sessionKey: sk,
                    count: drainBuffer.length,
                  });
                  scheduleResultBoundary(() => completeResult(msg), DRAIN_QUIET_MS);
                  return;
                }
                ...
                if (
                  ...
                  log.info("waiting for delayed proxy call at turn-result boundary", {
                    sessionKey: sk,
                    graceMs: PROXY_RESULT_BOUNDARY_GRACE_MS,
                  });
                  scheduleResultBoundary(() => completeResult(msg), PROXY_RESULT_BOUNDARY_GRACE_MS);
                  return;
                }
          ...
          const closeHandler = () => {
            ...
            if (drainBuffer.length > 0 || getPendingProxyCalls(sk).length > 0) {
              rejectAllPendingProxyCallsForSession(
                sk,
                new Error("Claude CLI subprocess closed before pending tool calls were resolved"),
              );
              drainBuffer.length = 0;
            }
            controllerClosed = true;
          ...
          const procErrorHandler = (err: Error) => {
            ...
            if (drainBuffer.length > 0 || getPendingProxyCalls(sk).length > 0) {
              rejectAllPendingProxyCallsForSession(
                sk,
                new Error(`Claude CLI subprocess error: ${err.message}`),
              );
              drainBuffer.length = 0;
            }
            controllerClosed = true;
          ...
          pendingProxyUnsubscribe = onPendingProxyCall(sk, (call) => {
            if (controllerClosed) {
              ...
              log.warn("pending proxy call arrived after stream close; rejecting", {
                sessionKey: sk,
                toolCallId: call.toolCallId,
                toolName: call.toolName,
              });
          ...
          if (options.abortSignal) {
            options.abortSignal.addEventListener("abort", () => {
              ...
              if (!hasReceivedContent) {
                log.info("abort signal received before content, closing stream immediately", {
                  cwd,
                });
                if (drainBuffer.length > 0 || getPendingProxyCalls(sk).length > 0) {
                  rejectAllPendingProxyCallsForSession(
              ...
              log.info("abort signal received mid-turn, starting grace period", { cwd });
          ...
          if (hasMatchedPendingResults) {
            ...
            for (const { call, result } of previousPendingProxyMatches) {
              if (result) {
                ...
                log.info("leaving unmatched parallel proxy call pending", {
                  sessionKey: sk,
                  toolCallId: call.toolCallId,
                  toolName: call.toolName,
                });
